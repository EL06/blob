'''
https://www.youtube.com/watch?v=D14YK-0MtcQ    , trouve le plus court chemin vers la solution
'''
...
ALL_COLORS = ['red', 'blue', 'yellow', 'black', 'green', 'purple', 'orange', 'cyan','magenta4','green4','aquamarine']
NTUBES = 0
LTUBES = 0
THIST = []
TVUES = []
...
class App(tk.Tk):                # définit la fenêtre self=window=tk.Tk
    def __init__(self ):        # on transmet le jeu et la liste des mouvements
        ...
        self.title('recherche de solution , méthode Breadth First Search, parcours en largeur')
...
class Tubes(ttk.Frame):
    ...
    def on_enter(self):
        ...
        if not TROUVE: 
            ...
            self.maj(u,v,k)
            if LMVT ==[]:
                textfin = 'Fin du jeu atteinte en '+str(self.nb_coups)+' coups'
...
def BFSearch(m):
    ...
    THIST.append((m,i))
    TVUES.append(m)
    dicSolution[i]=[0,(0,0,0)]
    
    while len(THIST)>0 and not TROUVE and i<10000 :
        #print('THIST ', THIST)
        s,p= THIST[0]
        THIST= THIST[1:]
        
        print('i est le père :',p)
        print('jeu ',s)
        
        if compare_m_m(s,MSOLUTION):
                TROUVE = True
                print('trouvé')
                print('la solution est en : ', p)
                chemin(p)
                exit
        
        # on cherche les jeux voisins possibles
        lmvt = cherche_mvt(s)
        print('liste des mouvements ', lmvt)
        # tant que existe mvt on applique ce mouvement
        while len(lmvt) > 0:
            
            u, v,k = lmvt.pop()
            
            print('mouvement ',u,v,k)
            test_essai = deplace(s,u,v,k)
            if dejavu_m(test_essai) :
                ...
                if i >10000:
                    exit
                THIST.append((test_essai,i))
                TVUES.append(test_essai)
                
                dicSolution[i]  = (p,(u,v,k))
                print('voici le dictionnaire chemin ', dicSolution)
...
def chemin(i):
    ...
    while mvt != (0,0,0):
        SOLUTION.append(mvt)
        n,mvt = dicSolution[n]
        print ('chemin de la fin vers le début', SOLUTION)
